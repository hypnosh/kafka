...
export class Kafka {
  constructor(canvasWidth, canvasHeight) {
    ...
    // Fixed horizontal position: 30% from left
    ...
    this.vx = 0;          // horizontal velocity (affects world scroll speed)
    this.onGround = true;
  ...
  _handleInput(input, dt) {
    ...
    if (attackHeld && (left || right) && this.onGround) {
      ...
      this.vx = right ? POUNCE_SPEED : -POUNCE_SPEED * 0.3;
    ...
    // Horizontal movement — affects world scroll speed
    if (this.state !== KAFKA_STATES.POUNCE) {
      if (right) {
        ...
        this.vx = WALK_SPEED * 0.4; // slowing/retreating
        this.facingRight = false;
        if (this.onGround) this.state = KAFKA_STATES.WALK;
      } else {
        this.vx = WALK_SPEED; // base auto-scroll speed contribution
